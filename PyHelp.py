...
#this is main file..
#wait.. ek file missing chhe..e mokli dau haa okk
...
def Exit():
    result = tkMessageBox.askquestion(
        'GALAXY: At Hand', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        window.destroy()
        exit()
...
def Home():
 ...
 Customize  search. Use-''', font=('Agency FB', 18),bg='white')
    ...
    try:
            ...
            b1 = Button(home, text="🔍", width=4, fg="PaleTurquoise", bg="Black", font=("Franklin Gothic Book",15,'bold'), relief="solid",bd=1, overrelief=RIDGE, borderwidth=4 ,command=search_fn)
            b1.place(x=950, y=35)
    except OSError:
            b1 = Button(home, text="🔍", width=4, fg="PaleTurquoise", bg="Black", font=("Franklin Gothic Book",15,'bold'), relief="solid",bd=1, overrelief=RIDGE, borderwidth=4 ,command=offline)
            b1.place(x=950, y=35)
    ...
    a = open('C:\\PyHelp\\randinfo.txt','r')
    s=a.read()
...
def search_fn():
    ...
    if 'lib:' in query:
                ...
                def done():
                     ...
                     for url in googlesearch.search(x):
                          links.append(url)
...
def Show():
    ...
    if vl =='Day':
            ...
            speak(f"Dear Sir or Madam, the time is {strTime}")
    ...
    elif vl == 'Encode_Decode_Message':
        import encode_decode

    elif vl == 'stopwatch':
        import watch

    elif vl == 'Music_player':
        import music_player


#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

window.title('GALAXY: At Hand')
...
option = OptionMenu(window, optionVar,'---Widgets---', "Time", "Day", "Calculator", "Calendar","Translator","Encode_Decode_Message","stopwatch","Music_player")
option.config(bg='PaleTurquoise', font=('arialnarrow', 15),borderwidth=4, relief = 'solid', width=61)
option.place(x=300,y=8)
...
try:
        socket.create_connection(('google.com',80))
        b = Button(window, text="🔍", width=4, fg="PaleTurquoise", bg="Black", font=("Franklin Gothic Book",18,'bold'), relief="solid",bd=2, overrelief=RIDGE, borderwidth=4,command=showhome)
        b.place(x=1100, y=360)
except OSError:

         b = Button(window, text="🔍", width=15, fg="PaleTurquoise", bg="Black", font=("Franklin Gothic Book",18,'bold'), relief="solid",bd=1, overrelief=RIDGE, borderwidth=4,command=offline)
         b.place(x=850, y=297)


speak('  welcome to Galaxy At Hand')
window.mainloop()
